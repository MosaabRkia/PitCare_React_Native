...
export default function ASK({ navigation ,route}) {
    ...
    const addQuestion =()=>navigation.navigate('AskQuestion',{catalog:route.params.catalog,fetchInBack});
    ...
    const fetchTheList =()=>{
      setIsLoadingFetch(true)
       fetch(`http://ruppinmobile.tempdomain.co.il/site28/api/getPosts/${route.params.catalog}/ASK`).then(r=>r.json()).then(data=>{  
        setData(data) 
        setIsLoading(false)  
    ...
    const search = async()=>{
      setIsLoadingFetch(true)
      await fetch(`http://ruppinmobile.tempdomain.co.il/site28/api/getPosts/${route.params.catalog}/ASK`).then(r=>r.json()).then(data=>{  
        let arr = []
...
isLoading? (
        ...
        <LinearGradient style={{width:"100%",height:"100%"}} colors={['#33F20C','#076175', '#061E79']}>
          <Navbar goBK={goBk} type="arrowWithTitle" title="ASK?"/> 
          {/* <SearchFutue /> */}
          <View> 
         <View>
         <TextInput
        ...
        <TouchableOpacity style={{position:'absolute',right:'8%',bottom:'5%',width:25,height:25}}  onPress={search}>
         <Icon
          ...
          data && data.map((e,index)=>{
            ...
            const ViewPage =()=>navigation.navigate('ViewPost',{post:e,back:"ASK",catalog:route.params.catalog});
          ...
          <Text style={{color:'white',fontSize:30,alignSelf:'center',fontWeight:"bold"}}>Add an ask Question</Text>  
          <TouchableOpacity 
          onPress={addQuestion}
  style={{ borderWidth:1,
