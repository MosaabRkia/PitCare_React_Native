...
export default function AskQuestion({navigation,route}) {
  const goBk =()=>navigation.navigate('ASK',{catalog:route.params.catalog});
...
const addPost = () =>{
  ...
  if(title === null){
    setCustomAlertText("please fill the title of question")
    setCustomAlert(true);
  ...
  if(description === null){
    setCustomAlertText("please fill the description of question")
    setCustomAlert(true);
    setisLoadingComment(false)
    return;
  }
...
).catch(err=>{console.log('error :' + err.message); setisLoadingComment(false)})
}
 ...
 const removePhoto =(e)=>{
    ...
    return (
         <LinearGradient style={{width:"100%",height:"100%"}} colors={['#33F20C','#076175', '#061E79']}>
             <Navbar goBK={goBk} type="arrowWithTitle" title="Ask Question"/> 
            <View 
            style={{
...
<View style={styles.divStyle}>
 ...
 <TextInput 
        ...
        style={styles.inputStyle}
         autoCompleteType="name"
         placeholder="description question clearly"
         placeholderTextColor="#ffffffb3"
        />
...
<Text style={{fontSize:25,fontWeight:'bold'}}>+</Text>
   ...
   {isLoadingComment?
   <Image alt="loading" style={{marginTop:10,width:100,height:80,alignSelf:'center'}} source={require('../assets/loadingGif.gif')}/>
   : <View style={{width:'70%',alignSelf:'center',marginTop:35}}>
   <ButtonWhiteTrans onPress={addPost} text="Submit Question"/>
   </View>}
